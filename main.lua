...
RegisterCustomEvent("HbkPrintToModLoader", function(ParamContext, ParamMessage)
    ...
    if Message:type() ~= "FString" then error(string.format("HbkPrintToModLoader Param #1 must be FString but was %s", Message:type())) end
...
RegisterCustomEvent("HbkConstructPersistentObject", function(ParamContext, ParamClass, OutParam)
    -- Param Type Checking
    local Class = ParamClass:get()
    if not Class:IsValid() then error("HbkConstructPersistentObject Param #1 must be a valid UClass") end
    if not UClassStaticRef then
        UClassStaticRef = StaticFindObject("/Script/CoreUObject.Class")
    end
    if not UClassStaticRef:IsValid() then error("HbkConstructPersistentObject could not find /Script/CoreUObject.Class") end
    if not Class:IsA(UClassStaticRef) then error("HbkConstructPersistentObject Param #1 must be a UClass or UClass derivative") end
    local OutValue = OutParam:get()
    if not OutValue:type() == "UObject" then error("HbkConstructPersistentObject Return Value must be a UObject") end
